...
成功空/未設定/取得失敗は所有者ごとに区別する。通信失敗直後からキャッシュにStaleを添え、取得日時を上部状態欄に表示する。取得失敗が未確定でも鮮度期限を超えた保存値はstale扱いにする。UI文字列は`No events today or tomorrow` / `Calendar not linked` / `Unavailable` / `Stale`。電力・買い物・プラグ・積算・料金のfieldは本MVPへ追加しない。
...
4. 当日分を終日→進行中→未来の開始時刻順に安定ソートし、3枠に満たない分だけ翌日分（終日→開始時刻順）で埋めて各3件を表示。時刻欄は日付を前置した`M/D all` / `M/D Now` / `M/D HH:mm`。`+N more`は当日の未表示件数のみを数える。
